...
const inventorsArr = inventors.map((inventor) => {
  return `${inventor.first} ${inventor.last}`;
});

// console.log(inventorsArr);
...
const longestLives = inventors.sort((a, b) => {
  return a.passed - a.year > b.passed - b.year ? -1 : 1;
});

// console.table(longestLives);
...
const transportation = data.reduce((object, item) => {
  // check if key exists, if not set to zero
  if (!object[item]) {
    object[item] = 0;
  }

  // if key exists - increment key count
  object[item]++;
  return object;
}, {});
